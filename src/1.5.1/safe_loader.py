...
def _compute_weight_stats(arr):
    ...
    try:
        ...
        # Drop NaN/Inf so stats remain stable on malformed tensors.
        ...
        # Use NumPy-only Shannon entropy to avoid optional scipy dependency.
        bins = min(256, max(8, flat.size // 10 + 1))
        hist, _ = np.histogram(flat, bins=bins)
        hist = hist.astype(np.float64)
        hist = hist[hist > 0]
        if hist.size > 0:
            probs = hist / np.sum(hist)
            weight_entropy = float(-np.sum(probs * np.log(probs + 1e-12)))
        else:
            weight_entropy = 0.0
