...
class Scanner:
    ...
    def _load_cache(self, sha: str, max_age_days: int = None):
        if max_age_days is None:
            max_age_days = self.CACHE_TTL_DAYS
        if not sha:
            return None
            ...
            if not self._is_cache_entry_fresh(entry, max_age_days=max_age_days):
                if self.verb:
                    self.logger.debug(
                        ...
                        extra={'sha': sha, 'max_age_days': max_age_days}
                    )
                return None
            return entry
        except Exception as e:
    ...
    def _run_security_parallel(self, path: str, type: str):
        ...
        for issue in self._issues_from_quick_signatures(self.results.get('quick_signatures', [])):
            if issue not in issues:
                issues.append(issue)
        self.results['security_issues'] = issues

    def _issues_from_quick_signatures(self, signatures: list) -> list:
        out = []
        if not signatures:
            return out
