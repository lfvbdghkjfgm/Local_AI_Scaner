...
class Outputer:
    ...
    def _text_format_single(results: Dict[str,Any]) -> str:
        ...
        signature_details = results.get('quick_signature_details', [])
        if signature_details:
            output.append(f"\nSIGNATURE MATCHES ({len(signature_details)}):")
            for i, detail in enumerate(signature_details[:12], 1):
                engine = str(detail.get('engine', 'n/a')).upper()
                sig_id = str(detail.get('id', 'unknown'))
                severity = str(detail.get('severity', 'n/a')).upper()
                category = str(detail.get('category', 'generic')).lower()
                output.append(f"  {i}. {engine} [{severity}/{category}] {sig_id[:80]}")
            if len(signature_details) > 12:
                output.append(f"  ... and {len(signature_details) - 12} more")
    ...
    def _json_summary_single(results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'counts': {
                ...
                'signature_matches': len(results.get('quick_signature_details', [])),
    ...
    def _csv_format_single(results: Dict[str,Any]) -> str:
        ...
        for key in ['format_threat', 'security_threats', 'backdoor_patterns', 'critical_threats', 
                    ...
                    'yara_critical', 'yara_high', 'yara_medium']:
