...
class Scanner:
    ...
    def __init__(self, out_form: str = 'text', verb: bool = False, risk_config: Dict[str, Any] = None):
        ...
        default_risk = {
            ...
            'weights': {
                'security_count': 1.2,           # Увеличено с 0.7
                'backdoor_count': 1.5,          # Увеличено с 0.8
                'warning_critical': 2.0,        # Увеличено с 0.5 - известные сигнатуры очень серьезны
                'network_ops': 1.5,             # Новый вес для сетевых операций
                'system_calls': 1.8             # Новый вес для опасных системных вызовов
            },
            'caps': {'security': 8.0, 'backdoor': 6.0, 'critical': 8.0},  # Увеличены лимиты
            'normalize_to': 10.0
        }
        ...
        try:
            if model_type == 'pytorch' and HAS_TORCH:
                ...
                for key, stats in tensor_stats.items():
                    ...
                    if kurtosis_val > 10:
                        backdoor_checks['suspicious_patterns'].append(f'High kurtosis in {key}: {kurtosis_val:.2f} (multimodal distribution)')
                        shadow_logic_score += 0.35
                    skewness_val = abs(stats.get('skewness', 0.0))
                    if skewness_val > 2:
                        backdoor_checks['suspicious_patterns'].append(f'Asymmetric distribution in {key}: skewness={skewness_val:.2f}')
                        shadow_logic_score += 0.15
                    analyzed += 1
                if shadow_logic_score > 0.5:
                    backdoor_checks['suspicious_patterns'].append(f'Shadow logic indicators detected (score: {shadow_logic_score:.2f})')
                    backdoor_checks['recommendations'].append('Verify model behavior on adversarial/OOD inputs to detect hidden triggers')
                ...
                if meta and meta.get('layers'):
                        ...
                        for w in weights:
                            ...
                            if shape and analyzed_layers < 3:
                                if len(shape) > 1 and shape[-1] > 1000:
                                    ...
                                    shadow_logic_score += 0.2
                                # Проверка статистики весов
                                weight_stats = w.get('stats', {})
                                if weight_stats:
                                    kurt = weight_stats.get('kurtosis', 0.0)
                                    skew = abs(weight_stats.get('skewness', 0.0))
                                    if kurt > 8:
                                        backdoor_checks['suspicious_patterns'].append(f"Layer {layer.get('name','?')} has bimodal weights (kurtosis={kurt:.1f})")
                                        shadow_logic_score += 0.3
                                    if skew > 1.5:
                                        shadow_logic_score += 0.15
                                analyzed_layers += 1
                    ...
                    if shadow_logic_score > 0.4:
                        backdoor_checks['suspicious_patterns'].append(f'Shadow logic patterns in weights (score: {shadow_logic_score:.2f})')
        except Exception as e:
    ...
    def calculate_risk(self):
        ...
        # Анализ критических угроз с разделением по типам
        critical_raw = 0.0
        network_ops_count = 0
        ...
        for msg in self.results.get('warnings', []):
            msg_lower = msg.lower()
            # Проверка известных сигнатур троянов - это критическая угроза
        ...
        # Также проверяем security_issues на троянов
        for issue in self.results.get('security_issues', []):
            ...
            if 'known trojan' in issue_lower:
                ...
                if network_ops_count == 0:  # Не дублируем подсчет
                    critical_raw += float(w.get('network_ops', 1.5))
                    network_ops_count += 1
            elif 'dangerous system call' in issue_lower:
                if system_calls_count < len(security):  # Не дублируем
                    critical_raw += float(w.get('system_calls', 1.8))
                    system_calls_count += 1
